...
<div class="container">
	...
	<c:if test="${user != null && user.me_id == board.bo_me_id}">
		...
		<a href="<c:url value='/board/update/${board.bo_num}'></c:url>">
			...
			<a href="<c:url value='/board/delete/${board.bo_num}'></c:url>"><button class="btn btn-outline-primary btn-delete">삭제</button></a>
		</div>
	</c:if>
</div>
<script>
$(function(){
	$('.btn-up, .btn-down').click(function(){
		if('${user.me_id}' == ''){
			...
			if(res){
				location.href="<c:url value='/member/login'></c:url>"
			}
		}
		let li_state = 1;
