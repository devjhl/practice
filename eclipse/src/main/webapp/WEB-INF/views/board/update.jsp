			...
			<div class="form-group mt-3 files">
				...
				<c:forEach items="${files}" var="file">
					<a class="form-control" href="<c:url value='/download${file.fi_name}'></c:url>" download="${file.fi_ori_name}">
						${file.fi_ori_name}
						<i class="btn-times" style="color:red;" data-num="${file.fi_num}">X</i> <!-- data-num 기본키 -->
					</a>
				</c:forEach>
...
$('.btn-times').click(function(e) {
	e.preventDefault();<!-- 버블링 효과 방지 a태그가 다운안됨-->
	$('.files').append('<input type="file" class="form-control" name="files">');
	$('.files').append('<input type="hidden" name="fileNums" value="'+$(this).data('num')+'">'); <!-- 서버에 전달해서 삭제해서 넣어줌--> 
	$(this).parent().remove(); <!--부모인 a 태그를 선택해서 삭제해줌-->
});
</script>